...
import { ToastContext, type ToastInternal, type ToastOptions, type ActionElement } from "./use-toast";
...
const Toast: React.FC<ToastInternal> = ({
  title,
  description,
  variant = "default",
  position = "top-right",
  ...
  const wrappedAction = action && React.isValidElement<{ onClick?: (e: React.MouseEvent) => void }>(action)
    ? React.cloneElement(action as ActionElement, {
